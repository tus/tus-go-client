...
func assertObservedBodyHeaders(
	label string,
	actual map[string]string,
	expected map[string]string,
) error {
	if !reflect.DeepEqual(actual, expected) {
		return fmt.Errorf("%s expected body headers %v, got %v", label, expected, actual)
	}

	return nil
}

func uploadWithBodyHeaders(
	ctx context.Context,
	...
	expectedBodyHeadersByMethod, err := api2devdock.UploadBodyHeadersByMethod(scenario)
	if err != nil {
		return nil, err
	}
	...
	bodyHeadersByMethod := map[string]map[string]string{}
		...
		tusgo.RequestLifecycleHooks{
			BeforeRequest: func(request *http.Request) error {
				...
				case http.MethodPost, http.MethodPatch:
					expectedHeaders, ok := expectedBodyHeadersByMethod[request.Method]
					if !ok {
						return nil
					}
					bodyHeadersByMethod[request.Method] = observedBodyHeaders(request, expectedHeaders)
				}
	...
	if upload == nil || upload.Location == "" {
		return nil, fmt.Errorf("upload body headers TUS upload did not expose an upload URL")
	}
	if upload.RemoteOffset != int64(len(content)) {
		return nil, fmt.Errorf(
			"upload body headers upload accepted %d bytes, expected %d",
			upload.RemoteOffset,
			len(content),
		)
	}
	if err := assertObservedBodyHeaders(
		"POST",
		bodyHeadersByMethod[http.MethodPost],
		expectedBodyHeadersByMethod[http.MethodPost],
	); err != nil {
		return nil, err
	}
	if err := assertObservedBodyHeaders(
		"PATCH",
		bodyHeadersByMethod[http.MethodPatch],
		expectedBodyHeadersByMethod[http.MethodPatch],
	); err != nil {
		return nil, err
	}

	return map[string]interface{}{
		"bodyHeadersByMethod": bodyHeadersByMethod,
...
func main() {
	...
	scenario, err := api2devdock.LoadScenario(
		"examples/api2-devdock-tus-upload-body-headers/api2-scenario.json",
	)
	if err != nil {
	...
	result, err := uploadWithBodyHeaders(ctx, scenario, createResponse)
	if err != nil {
		api2devdock.Fail("upload body headers: %v", err)
	}
	...
	fmt.Printf("Go TUS SDK devdock scenario %s observed body headers to %s\n", scenarioID, result["uploadUrl"])
}
